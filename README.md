...
- **Follow a flight, and know when to leave**: enter a flight number, arrival airport and your travel time, and the display becomes a countdown — ETA, distance to go, phase (waiting / enroute / descending / approaching / landed), and then **LEAVE NOW** at the right moment. It uses a direct callsign lookup on a cadence that tightens as the flight nears, and it accounts for the gap between touchdown and the person actually walking out (taxi, deplaning, immigration, bags — usually 20–45 minutes), because an alert keyed to landing alone sends you to the airport far too early. It is careful about what it claims: the ETA comes from current groundspeed, **not** a published schedule, and lost signal is reported as lost signal rather than as an arrival. See `docs/DISPLAY_UI.md`.
