...
Consumed by `services/gateway` and `apps/tablet-pwa` so there is exactly one definition of each model in the TypeScript world. Firmware (C++) mirrors these by hand in `firmware/display/include/domain/protocol.h` — see `docs/PROTOCOL.md` for the cross-language contract of record.
