...
| "Leave now" prompt | Departure prompt for a pickup | Yes | Yes | No | None | 3 | done | Native (7 departure-advice) + PWA (4) | Takes over the primary metric cell so it can't be missed, and accounts for **touchdown-to-walk-out time**, not just landing — an alert keyed to touchdown alone sends people to the airport 20–45 min early. Deliberately *not* built on `AlertRule`: one subtraction with no cooldown, channels or match expression doesn't need a rule engine |
| General alert rules (area entry, aircraft type, emergency squawk, …) | Category feature | No | No | No | None | 3 | future | — | `AlertRule` model still stubbed and unimplemented. The departure prompt above pointedly does not use it |
