...
| **adsb.lol** | Free | None currently required (confirmed live) | Not fixed/published; described as dynamic based on load. The project has signaled a future move toward requiring an API key — re-check before relying on this long-term. | ODbL 1.0 (open data) — hobby/community-friendly, arose as an open alternative after ADS-B Exchange's acquisition | No — HTTPS only, and TLS-on-ESP32 heap headroom is unverified without physical hardware, so we don't risk it in Phase 1 | Community-run, generally reliable, no SLA — **confirmed working against real traffic** in this project's own testing | **Phase 1 live provider**, via the gateway (never directly from firmware) |
...
**Confirmed working**, not just designed: the `/v2/point/{lat}/{lon}/{radiusNm}` endpoint the adapter guesses at (`services/gateway/src/providers/adsblol.ts`) was verified directly against the live API and returns exactly the field shape (`hex`, `flight`, `r`, `t`, `alt_baro`, `alt_geom`, `gs`, `track`, `baro_rate`, `squawk`, `emergency`, `category`, `lat`, `lon`, ...) the normalizer expects. End-to-end tested with a real M5Stack Core2: switching `AVIATION_PROVIDER=adsblol` and pointing at a real home location showed real, moving commercial traffic (confirmed against multiple different real flights over time) on both the Core2 screen and the tablet PWA.
